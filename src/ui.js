 ...
 * @param {string} [coin] — secondary badge text
 */
export function toast(title, coin) {
...
export function render() {
  ...
  // Show/Hide Bank (NGN) option based on country location
  const isNG = !S.country || S.country === 'NG';
  const methodBankEl = $('method-bank');
  if (methodBankEl) {
    methodBankEl.style.display = isNG ? '' : 'none';
  }

  // Force selection to USDT if user is not in Nigeria and Bank (NGN) is selected
  if (!isNG && (!S._selectedMethod || S._selectedMethod.name === 'Bank (NGN)')) {
    setLocal('_selectedMethod', { name: 'USDT', min: 150000 });
  ...
  const selectedMethod = S._selectedMethod || { name: 'Bank (NGN)', min: 50000 };
  ...
  if (wFiatEl) {
    if (isNG) {
      wFiatEl.textContent = '₦' + fmt(effectiveBalance * ORL_TO_NGN, 2);
    } else {
      wFiatEl.textContent = '$' + fmt(effectiveBalance / 50000, 2);
    }
  }
  ...
  if (spinTagEl) spinTagEl.textContent = 'Free with ad';

  const spinBtnEl = $('spinBtn');
  if (spinBtnEl) spinBtnEl.textContent = 'Spin the wheel';

  const scratchTagEl = $('scratchTag');
  if (scratchTagEl) scratchTagEl.textContent = `${S.scratchLeft || 0} left today`;
